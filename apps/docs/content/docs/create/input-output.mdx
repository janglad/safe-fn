...
<Callout type="warn">
  ...
  Since the input is not parsed and the type is expanded with the
  `unsafeRawInput` of the child, the parent function will be called with
  properties that only appear in the type of the child function!
</Callout>
